...
class Downloader {
  ...
  public async checkCapabilities(testArticleId: string='MediaWiki:Sidebar'): Promise<void> {
    ...
    // fail the check.
    this.mwCapabilities.mobileRestApiAvailable = await this.checkApiAvailabilty(this.mw.mobileRestApiUrl.href + encodeURIComponent(testArticleId.replace(/ /g,'_') || 'Main_Page'));
    this.mwCapabilities.desktopRestApiAvailable = await this.checkApiAvailabilty(this.mw.desktopRestApiUrl.href + encodeURIComponent(testArticleId.replace(/ /g,'_')));
